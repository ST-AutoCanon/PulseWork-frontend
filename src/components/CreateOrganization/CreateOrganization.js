...
const MultiSelectCheckbox = ({ options, selectedValues, onChange, disabled }) => {
  ...
  return (
    <div className={`orgprefix-multi-select-container ${disabled ? "orgprefix-disabled" : ""}`}>
      <div
...
const CreateOrganization = ({ employeeId = "default-employee-id" }) => {
  ...
  // Fetch sidebar menu items (unchanged)
  useEffect(() => {
    const fetchSidebarItems = async () => {
      try {
        ...
        showAlert("Failed to fetch sidebar menu items. Please try again or contact support.", "Error");
        setSidebarItems([]);
        setSidebarAccess([]);
      }
    };
  ...
  // Fetch sidebar access for editing (unchanged)
  useEffect(() => {
    const fetchSidebarAccess = async () => {
      if (isEditing && currentOrgId && sidebarItems.length > 0) {
        try {
          ...
          showAlert("Failed to fetch sidebar access. Please try again or contact support.", "Error");
        }
      }
    };
  ...
  // Filter organizations based on search term (unchanged)
  useEffect(() => {
  ...
  const handleDelete = async (orgId) => {
    ...
    try {
      ...
      if (response.ok) {
        showAlert(data.message || "Organization deleted successfully.", "Success");
        setOrgTableData((prev) => prev.filter((org) => org.id !== orgId));
        setFilteredOrgData((prev) => prev.filter((org) => org.id !== orgId));
      } else {
        showAlert(data.message || `Failed to delete organization (status: ${response.status}).`, "Error");
      }
    } catch (error) {
      console.error("Delete organization error:", error);
      showAlert(`Network error: ${error.message}. Please check your connection or server status.`, "Error");
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const data = await response.json();
      if (response.ok) {
        showAlert(`Organization ${isEditing ? "updated" : "created"} successfully.`, "Success");
        if (isEditing) {
        ...
        showAlert(data.error || `Failed to ${isEditing ? "update" : "create"} organization.`, "Error");
      }
    } catch (error) {
  ...
  return (
    <div className="orgprefix-create-org-wrapper">
        ...
        <div className="orgprefix-modal-overlay" onClick={handleCloseModal}>
          <div className="orgprefix-create-org-container">
            <div className="orgprefix-form-header">
              ...
              <span className="orgprefix-close-icon" onClick={() => setShowForm(false)}>
                ✕
              </span>
            </div>
                ...
                <div className="orgprefix-form-section">
                  ...
                  <div className="orgprefix-form-row">
                    <div className="orgprefix-form-field">
                      ...
                      />
                      {errors.name && <span className="orgprefix-error-message">{errors.name}</span>}
                    </div>
                    <div className="orgprefix-form-field">
                      ...
                      />
                      {errors.subdomain && <span className="orgprefix-error-message">{errors.subdomain}</span>}
                    </div>
                    <div className="orgprefix-form-field">
                      ...
                      />
                      {errors.noEmployees && <span className="orgprefix-error-message">{errors.noEmployees}</span>}
                    </div>
                  </div>
                  <div className="orgprefix-form-row">
                    <div className="orgprefix-form-field">
                      ...
                      />
                      {errors.companyAddress && <span className="orgprefix-error-message">{errors.companyAddress}</span>}
                    </div>
                    <div className="orgprefix-form-field">
                      ...
                      />
                      {errors.cPanNo && <span className="orgprefix-error-message">{errors.cPanNo}</span>}
                    </div>
                    <div className="orgprefix-form-field">
                      ...
                      />
                      {errors.adminEmail && <span className="orgprefix-error-message">{errors.adminEmail}</span>}
                    </div>
                  </div>
                  <div className="orgprefix-form-row orgprefix-form-row-four">
                    <div className="orgprefix-form-field">
                      ...
                      />
                      {errors.contactEmail && <span className="orgprefix-error-message">{errors.contactEmail}</span>}
                    </div>
                    <div className="orgprefix-form-field">
                      ...
                      />
                      {errors.contactPhone && <span className="orgprefix-error-message">{errors.contactPhone}</span>}
                    </div>
                    <div className="orgprefix-form-field orgprefix-date-field">
                      ...
                      {errors.startDate && <span className="orgprefix-error-message">{errors.startDate}</span>}
                    </div>
                    <div className="orgprefix-form-field orgprefix-date-field">
                      ...
                      {errors.endDate && <span className="orgprefix-error-message">{errors.endDate}</span>}
                    </div>
                  </div>
                  <div className="orgprefix-form-actions">
                    <button type="button" className="orgprefix-cancel-btn" onClick={() => setShowForm(false)}>
                      Cancel
                    </button>
                    <button type="button" className="orgprefix-next-btn" onClick={handleNextStep}>
                      Next
                    </button>
                  </div>
                </div>
                      ...
                      {sidebarItems.length > 0 ? (
                        <div className="orgprefix-sidebar-list">
                          {sidebarItems.map((item) => {
                            ...
                            return (
                              <div key={item.id} className="orgprefix-sidebar-item">
                                <span className="orgprefix-sidebar-label">{item.label}</span>
                                <MultiSelectCheckbox
                                  options={roles}
                                  selectedValues={selectedRoles}
                                  onChange={(newRoles) => handleSidebarRoleChange(item.id, newRoles)}
                                  disabled={alertModal.message.includes("Failed to fetch sidebar menu items")}
                                />
                        ...
                        <p className="orgprefix-no-data">
                          {alertModal.message.includes("Failed to fetch sidebar menu items")
                            ? "Unable to load sidebar items. Please try again or contact support."
                            : "No sidebar items found."}
                        </p>
                      )}
                      {errors.sidebarAccess && (
                        <span className="orgprefix-error-message">{errors.sidebarAccess}</span>
                      )}
                    </div>
                  </div>
                  <div className="orgprefix-form-actions">
                    <button type="button" className="orgprefix-prev-btn" onClick={handlePrevStep}>
                      Previous
                    </button>
                    <button
                      type="submit"
      ...
      {showDetailsPopup && popupData && (
        <div className="orgprefix-modal-overlay" onClick={handleCloseDetailsPopup}>
          <div className="orgprefix-create-org-container orgprefix-details-container">
            <div className="orgprefix-form-header">
              <h2>Organization Details</h2>
              <span className="orgprefix-close-icon" onClick={() => setShowDetailsPopup(false)}>
                ✕
              </span>
            </div>
          ...
          <div className="orgprefix-mobile-cards">
            {filteredOrgData.map((org) => (
              <div className="orgprefix-org-card" key={org.id}>
                ...
                <div className="orgprefix-org-card-content"><strong>ID:</strong> {org.id}</div>
                <div className="orgprefix-org-card-content"><strong>Subdomain:</strong> {org.subdomain}</div>
                <div className="orgprefix-org-card-content"><strong>No. Employees:</strong> {org.no_employees}</div>
                <div className="orgprefix-org-card-actions">
                  <button className="orgprefix-view-btn" onClick={() => handleShowDetails(org)} title="View Details">
                    <FaEye />
                  </button>
                  <button className="orgprefix-edit-btn" onClick={() => handleEdit(org)} title="Edit">
                    <MdEdit />
                  </button>
                  <button className="orgprefix-delete-btn" onClick={() => handleDelete(org.id)} title="Delete">
                    <FaTrash />
                  </button>
                </div>
              </div>
          ...
          <div className="orgprefix-org-table-container">
            <table className="orgprefix-org-table">
              ...
              <tbody>
                {filteredOrgData.map((org) => (
                  <tr key={org.id}>
                    <td><span className="orgprefix-tooltip" title={org.id}>{org.id}</span></td>
                    <td><span className="orgprefix-tooltip" title={org.Name}>{org.Name}</span></td>
                    <td><span className="orgprefix-tooltip" title={org.subdomain}>{org.subdomain}</span></td>
                    <td><span className="orgprefix-tooltip" title={org.no_employees.toString()}>{org.no_employees}</span></td>
                    <td>
