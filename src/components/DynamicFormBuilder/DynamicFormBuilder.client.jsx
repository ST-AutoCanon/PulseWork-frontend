  ...
  try {
    ...
    if (isOthersOnlyMode) {
      ...
      if (requesterResponse?.response_json) {
        // Load requester's full response so context is visible
        setFormData(requesterResponse.response_json || {});

        const baseField = othersFeedbackContext.fieldId;
        const feedbackKey = `${baseField}_others_feedback_from_${currentEmployeeId}`;

        const existingFeedback = requesterResponse.response_json?.[feedbackKey];

 if (existingFeedback && String(existingFeedback).trim() !== "") {
  setSubmittedFeedbackData({
    feedbackText: existingFeedback,
    requesterName:
      othersFeedbackContext.requesterName || "Unknown",
    timestamp:
      employeeResponse?.submitted_at || new Date().toISOString(),
  });
...
const renderField = (field, isPreview = true, onChange = null) => {
  ...
  return (
    <div className="df-form-group">
      ...
      {/* Show saved files - Real filename only */}
      {Array.isArray(currentValue) && currentValue.length > 0 && (
        <div style={{ marginTop: "12px" }}>
          {currentValue.map((file, idx) => {
            // Strong real filename extraction - NO "File 1" fallback
            const name = 
              file?.originalname || 
              file?.filename || 
              file?.name || 
...
{!isNotSubmitted && isExpanded && (
  ...
  metaKey.toLowerCase().includes("savedat")
) {
  displayValue = metaValue 
    ? new Date(metaValue).toLocaleString("en-IN", { 
        timeZone: "Asia/Kolkata",
