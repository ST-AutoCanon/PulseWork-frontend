...
export default function ExitFlow() {
  ...
  const fetchAllTeamRequests = async () => {
    try {
      ...
      console.log(
        "[TEAM REQUESTS] Pending normal:",
        requests.filter(
          (r) => r.supervisor_status === "PENDING" || r.hr_status === "PENDING",
        ).length,
      );
      console.log(
        "[TEAM REQUESTS] Pending withdraw:",
        requests.filter(
          (r) =>
            r.withdrawal_requested_at &&
            r.withdrawal_supervisor_status === "PENDING",
        ).length,
      );
      console.log(
        "[TEAM REQUESTS] Resigned:",
        requests.filter((r) => r.final_outcome === "RESIGNED").length,
      );
      setAllTeamRequests(requests);
      setPendingData({
        normal: requests.filter(
          (r) => r.supervisor_status === "PENDING" || r.hr_status === "PENDING",
        ),
  ...
  const handleApply = async () => {
    ...
    if (!form.proposedLwd)
      return setErrorMessage("Please select proposed Last Working Day");
    setLoading(true);
    try {
      await axios.post(
  ...
  const handleWithdraw = async () => {
    ...
    if (!withdrawReason.trim())
      return setErrorMessage("Please enter withdrawal reason");
    setLoading(true);
    try {
      await axios.post(
      ...
      if (reviewType === "normal") {
        if (effectiveRole === "hr" || effectiveRole === "admin") {
          if (action === "APPROVED") {
            if (!finalLwd) {
              setErrorMessage(
                "Final Last Working Day is required to approve resignation",
              );
              setLoading(false);
              return;
            }
                                  ...
                                  {ktPlans.length === 0 ? (
                                    <div className="text-center py-12 bg-gray-50 rounded-lg border border-dashed">
                                      <p className="text-gray-500 mb-3">
                                        No KT plans added yet.
                                      </p>
                                      <p className="text-sm text-gray-600">
                                        Add your handover topics and upload
                                        related documents.
                                      </p>
                                    </div>
                                  ) : (
                                        ...
                                        >
                                          <div className="flex justify-between items-start">
                                            <div className="flex-1">
                                              ...
                                              {kt.attached_files &&
                                                kt.attached_files.length >
                                                  0 && (
                                                  <div className="mt-2 space-y-2">
                                                    <p className="text-xs font-medium text-gray-700">
                                                      Uploaded files:
                                                    </p>
                                                    {kt.attached_files.map(
                                                      (file, idx) => {
                                                        const fileName = file
                                                          .split("/")
                                                          .pop();
                                    ...
                                    {assets.map((asset) => (
                                      ...
                                      >
                                        ...
                                        <div className="mt-2 text-sm">
                                          Supervisor Approved:{" "}
                                          {asset.supervisor_approved
                                            ? "Yes"
                                            : "No"}
                                          <br />
                                          HR Approved:{" "}
                                          {asset.hr_approved ? "Yes" : "No"}
                                        </div>
                                      </div>
                                    ))}
                                  </div>
              ...
              <>
                  ...
                  {allTeamRequests.length === 0 ? (
                    ...
                    <div className="exf-table-container">
                      <table className="exf-table">
                        ...
                        <tbody>
                          {allTeamRequests
                            .filter((req) => !req.withdrawal_requested_at)
                            .map((req) => (
                              <tr key={req.id}>
                ...
                {["hr", "supervisor", "manager"].includes(role) && (
                  ...
                  <div className="exf-team-panel mt-8 border-t border-gray-200 pt-6">
                    <h2 className="exf-panel-title mb-4">
                      Resigned Employees & Clearance Status (
                      {resignedClearance.length})
                    </h2>

                    {resignedClearance.length === 0 ? (
                      <div className="exf-empty-state">
                        No resigned employees requiring clearance in the
                        organization
                      </div>
                    ) : (
                      <div className="exf-table-wrapper">
                  ...
                  const hrPendingWithdrawals = allTeamRequests.filter(
                    (r) =>
                      ...
                      (r.withdrawal_hr_status === "PENDING" ||
                        !r.withdrawal_hr_status), // HR not yet acted
                  );
            ...
            <div className="exf-modal-content">
                ...
                {selectedRequest?.type === "normal" && role === "hr" && (
                  <div className="mt-6 p-5 bg-gray-50 rounded-lg border border-gray-200">
                    ...
                    <p className="mt-3 text-sm text-gray-600">
                      This will become the employee's official last working day
                      upon final approval.
                    </p>
                  </div>
                )}
              ...
              {selectedRequest.type === "clearance" && (
                <div className="exf-clearance-section mt-6 space-y-8">
                  {/* KT Plans */}
                  <div>
                    <h4 className="exf-section-title">
                    ...
                    {assets.length === 0 ? (
                      ...
                      assets.map((asset) => (
                        <div key={asset.id} className="exf-card relative">
                          ...
                          {loading && (
                            <div className="exf-loading-overlay">Saving...</div>
                          )}
                        </div>
                      ))
                    )}
