...
export default function ExitFlow() {
  ...
  const isHr =
    ...
    roleLower === "hr admin";
  const isSupervisorLike = ["supervisor", "manager", "team lead"].includes(
    roleLower,
  );
  ...
  const fetchClearanceItems = async (exitId) => {
    ...
    try {
      ...
      const res = await axios.get(
        `${BACKEND_URL}/api/clearance/${exitId}/items`,
        {
          headers: {
            "x-api-key": API_KEY,
            "x-employee-id": employeeId,
  ...
  const fetchAllTeamRequests = async () => {
    try {
      ...
      console.log(
        "[TEAM REQUESTS] All statuses:",
        requests.map((r) => ({
          id: r.id,
          emp_id: r.employee_id,
          supervisor_status: r.supervisor_status,
          hr_status: r.hr_status,
          withdrawal_supervisor_status: r.withdrawal_supervisor_status,
          withdrawal_hr_status: r.withdrawal_hr_status,
        })),
      );
      ...
      // Filter pending data based on role
      const effectiveRole =
        role?.toLowerCase() === "manager" ? "supervisor" : role?.toLowerCase();
      ...
      filteredPendingNormal = requests.filter(
        (r) =>
      ...
      filteredPendingWithdraw = requests.filter(
        (r) =>
          r.withdrawal_requested_at &&
          !["APPROVED", "REJECTED"].includes(
            r.withdrawal_supervisor_status || "",
          ) &&
          !["APPROVED", "REJECTED"].includes(r.withdrawal_hr_status || ""),
      );
      console.log("[TEAM REQUESTS] Effective role:", effectiveRole);
      console.log(
        "[TEAM REQUESTS] Filtered pending normal:",
        filteredPendingNormal.length,
        "items",
      );
      console.log(
        "[TEAM REQUESTS] Filtered pending withdraw:",
        filteredPendingWithdraw.length,
        "items",
      );
      console.log("[TEAM REQUESTS] Effective role:", effectiveRole);
      console.log(
      ...
      if (filteredPendingNormal.length > 0) {
        console.log(
          "[TEAM REQUESTS] First pending normal:",
          filteredPendingNormal[0],
        );
      }
      setPendingData({
      ...
      setResignedClearance(
        requests.filter((r) => r.final_outcome === "RESIGNED"),
      );
    } catch (err) {
      console.error(
        "[TEAM REQUESTS FETCH ERROR]",
        err.response?.data || err.message,
      );
    }
  };
  ...
  const handleAddKt = async () => {
    ...
    try {
      ...
      if (newKtForm.completedDate)
        formData.append("plannedDate", newKtForm.completedDate);
      ...
      const msg =
        err.response?.data?.error || err.message || "Failed to add KT plan";
      setErrorMessage(msg);
      showAlert("Error: " + msg);
    } finally {
  ...
  const handleApproveItem = async (itemId, approved, itemType) => {
    ...
    try {
      ...
      console.log("[Frontend] Approving as:", approvalAs, {
        itemId,
        approved,
        itemType,
      });
  ...
  const handleFinalizeClearance = async () => {
    ...
    try {
      ...
      showAlert(
        "Clearance process finalized successfully!",
        "Success",
        "success",
      );
      ...
      setErrorMessage(
        err.response?.data?.error || "Failed to finalize clearance",
      );
  ...
  const handleFinalizeExit = async () => {
    ...
    try {
      ...
      if (res.data?.success) {
        ...
        throw new Error(
          res.data?.error || "Finalize failed - no success response",
        );
      }
    } catch (err) {
      setErrorMessage(
        err.response?.data?.error || err.message || "Failed to finalize exit",
      );
      showAlert(
        "Finalize failed: " + (err.response?.data?.error || "Check console"),
      );
    } finally {
      setLoading(false);
    }
  ...
  const handleReviewAction = async (reviewType, action) => {
    ...
    try {
        ...
        if (isTeamTab) {
          ...
          if (effectiveRole === "hr" || effectiveRole === "admin") {
            if (action === "APPROVED") {
              if (!finalLwd) {
                setErrorMessage(
                  "Final Last Working Day is required to approve",
                );
                setLoading(false);
                return;
              }
      ...
      const msg =
        err.response?.data?.error || err.message || "Failed to process review";
      setErrorMessage(msg);
      showAlert(msg, "Error", "error");
    } finally {
      setLoading(false);
                      ...
                      <div className="withdrawal-focused-card bg-white p-6 rounded-lg border border-gray-200 shadow-sm">
                          ...
                          <div>
                            ...
                            <span
                              className={`inline-block mt-1 px-3 py-1 rounded-full text-sm font-medium badge badge-${(
                                selfRequest.withdrawal_supervisor_status ||
                                "pending"
                              ).toLowerCase()}`}
                            >
                              {selfRequest.withdrawal_supervisor_status ||
                        ...
                        <div className="mt-6 p-4 bg-blue-50 border border-blue-100 rounded-lg">
                          <p className="text-sm text-blue-800">
                            <strong>
                              You have requested to withdraw your resignation.
                            </strong>
                          </p>
                    ...
                    <>
                      ...
                      <div className="exf-status-grid">
                        ...
                        <div className="exf-status-item">
                          ...
                          <span
                            className={`badge badge-${(selfRequest.supervisor_status || "pending").toLowerCase()}`}
                          >
                        ...
                        {selfRequest.supervisor_comment && (
                          <div className="exf-status-item exf-full-width">
                            <span className="exf-status-label">
                              Supervisor Comment
                            </span>
                        ...
                        <div className="exf-status-item">
                          ...
                          <span
                            className={`badge badge-${(selfRequest.hr_status || "pending").toLowerCase()}`}
                          >
                      ...
                      {!selfRequest.final_outcome && (
                        <div className="exf-withdraw-section mt-6">
                          ...
                          <p className="exf-help-text">
                            You can still cancel this resignation before final
                            approval.
                          </p>
                          <textarea
                            ...
                            <div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                              ...
                              {selfRequest.leave_policy === "all" && (
                                <p className="text-sm text-blue-700">
                                  You are allowed to encash/adjust{" "}
                                  <strong>all eligible leaves</strong>
                                  (including earned/privilege leaves) as per
                                  company policy.
                                </p>
                              )}
                              {selfRequest.leave_policy === "sick_only" && (
                                <p className="text-sm text-blue-700">
                                  Only <strong>sick and casual leaves</strong>{" "}
                                  will be considered for adjustment. No
                                  encashment of earned/privilege leaves is
                                  allowed.
                                </p>
                              )}
                              {selfRequest.leave_policy === "none" && (
                                <p className="text-sm text-red-700">
                                  <strong>No leaves</strong> will be encashed or
                                  adjusted during your exit process.
                                </p>
                              )}
                          ...
                          {!selfRequest.leave_policy &&
                            selfRequest.final_lwd && (
                              <p className="mt-4 text-sm text-gray-600">
                                Leave policy details will be communicated by HR
                                shortly.
                              </p>
                            )}
                          <div className="mt-6 pt-5 border-t border-gray-200">
                            ...
                            {exitCompleted ? (
                              ...
                              <button
                                onClick={async () => {
                                  console.log(
                                    "[EMPLOYEE] Opening clearance modal, fetching data...",
                                  );
                                  setShowClearanceModal(true);
                                  if (selfRequest?.id) {
                                    console.log(
                                      "[EMPLOYEE] Fetching items for exit ID:",
                                      selfRequest.id,
                                    );
                                    await fetchClearanceItems(selfRequest.id);
                                    console.log(
                                      "[EMPLOYEE] Items fetched successfully",
                                    );
                                  } else {
                                    console.error(
                                      "[EMPLOYEE] selfRequest.id not available!",
                                    );
                                  }
                                }}
                    ...
                    <div className="exf-resignation-wrapper-1">
                      {!showResignationForm ? (
                        <div className="exf-resignation-apply-area">
                          <h2 className="exf-section-title">
                        ...
                        <div className="exf-resignation-form-container">
                          <div className="exf-resignation-form-header-1">
                            <h3 className="exf-resignation-form-title">
                              Resignation Details
                            </h3>
                            <button
                              type="button"
                ...
                <>
                  <div className="exf-team-panel">
                    <h2 className="exf-panel-title mb-4">
                      Exit & Withdrawal Requests from My Team
                    </h2>
                    {allTeamRequests.length === 0 ? (
                      <div className="exf-empty-state">
                        No exit or withdrawal requests submitted by your team
                        members yet.
                      </div>
                    ) : (
                          ...
                          <tbody>
                            {pendingData.withdraw.map((req) => (
                              <tr key={req.id}>
                                ...
                                <td>
                                  <span
                                    className={`exf-badge exf-badge-${(req.withdrawal_supervisor_status || "pending").toLowerCase()}`}
                                  >
                                    {req.withdrawal_supervisor_status ||
                                      "Pending"}
                                  </span>
            ...
            (role === "hr" || role === "admin" || isAdmin) && (
              <div className="exf-team-view space-y-8">
                <div className="exf-team-panel">
                  ...
                  {allTeamRequests.length === 0 ? (
                    <div className="exf-empty-state">
                      No exit or withdrawal requests found across the
                      organization.
                    </div>
                  ) : (
                ...
                <div className="exf-team-panel mt-8 border-t pt-6">
                  <h2 className="exf-panel-title mb-3">Withdrawal Requests</h2>
                  {(() => {
                    console.log(
                      "[DEBUG] Withdrawal candidates:",
                      allTeamRequests
                ...
                {selectedRequest?.type === "normal" && (isHr || isAdmin) && (
                  <div className="hr-decision-section">
                    ...
                    {(isAdmin ||
                      ...
                      <div className="info-card decision-card">
                        ...
                        <p className="helper-text">
                          This will become the employee's official last working
                          day upon final approval.
                        </p>
                      </div>
                    )}
                    {/* Leave Policy */}
                    {(isAdmin ||
                      ...
                      <div className="info-card decision-card">
                        <label className="decision-label">
                          Leave Encashment / Adjustment Policy{" "}
                          <span className="required">*</span>
                        </label>
                        ...
                        <p className="helper-text italic">
                          This setting will be used for final leave settlement
                          calculation.
                        </p>
                      </div>
                    )}
                                ...
                                <div className="files-list">
                                  ...
                                  {kt.attached_files.map((file, idx) => {
                                    ...
                                    return (
                                      <div
                                        key={idx}
                                        className="exit-clearance-attachment-row"
                                      >
                                        <span className="exit-clearance-attachment-name">
                                          {fileName}
                                        </span>
                                ...
                                {["supervisor", "manager"].includes(role) && (
                                  <label className="checkbox-label">
                                    <input
                                      type="checkbox"
                                      checked={
                                        asset.supervisor_approved || false
                                      }
                                      onChange={(e) =>
                                        handleApproveItem(
                                          asset.id,
                    ...
                    {activeTab === "all" &&
                      ...
                      !isAdmin && (
                        <div className="warning-message mb-3 text-yellow-700">
                          This request is still pending supervisor review.
                          <br />
                          You can view details, but final approval requires
                          supervisor action first.
                        </div>
                      )}
                    ...
                    {(activeTab === "team" ||
                      ...
                      <>
                        <button
                          ...
                          onClick={() =>
                            handleReviewAction("normal", "APPROVED")
                          }
                          disabled={
                            loading ||
                            (activeTab !== "team" &&
                        ...
                        <button
                          ...
                          onClick={() =>
                            handleReviewAction("normal", "REJECTED")
                          }
                          disabled={loading}
                        >
                ...
                {selectedRequest.type === "withdrawal" && (
                  <div className="action-buttons">
                    <button
                      className="btn btn-success"
                      onClick={() =>
                        handleReviewAction("withdrawal", "APPROVED")
                      }
                      disabled={loading}
                    >
                      {activeTab === "team"
                    ...
                    <button
                      className="btn btn-danger"
                      onClick={() =>
                        handleReviewAction("withdrawal", "REJECTED")
                      }
                      disabled={loading}
                    >
                      {activeTab === "team"
              ...
              <div className="exf-modal-content" style={{ maxWidth: "520px" }}>
                <div className="exf-kt-modal-header">
                  <h3 className="exf-kt-modal-title">
                    {editingKtId
                      ? "Edit Knowledge Transfer Plan"
                      : "Add Knowledge Transfer Plan"}
                  </h3>
                  <button
                    onClick={() => {
                      setShowAddKTModal(false);
                ...
                <div className="exf-kt-modal-body">
                  ...
                  <div className="exf-kt-form-group">
                    ...
                    <textarea
                      value={
                        editingKtId
                          ? editKtForm.description
                          : newKtForm.description
                      }
                      onChange={(e) =>
                        editingKtId
                          ? setEditKtForm({
                  ...
                  <div className="exf-kt-form-group">
                    ...
                    <div className="exf-kt-file-upload-container">
                      ...
                      <span className="exf-kt-file-selected">
                        {(editingKtId
                          ? editKtForm.documents
                          : newKtForm.documents
                        ).length > 0
                          ? `Selected: ${(editingKtId
                              ? editKtForm.documents
                              : newKtForm.documents
                            )
                              .map((f) => f.name)
                    ...
                    {editingKtId &&
                      ktPlans.find((k) => k.id === editingKtId)?.attached_files
                        ?.length > 0 && (
                        <div className="mt-4">
                          ...
                          {ktPlans
                            .find((k) => k.id === editingKtId)
                            .attached_files.map((rawPath, idx) => {
                              const fileName =
                                normalizeFilePath(rawPath).split("/").pop() ||
                                "file";
                              const isMarkedDelete =
                                editKtForm.filesToDelete.includes(rawPath);
                              return (
                                <div
                                  key={idx}
                                ...
                                >
                                  <span
                                    className={`text-sm ${isMarkedDelete ? "line-through text-gray-500" : "text-gray-700"}`}
                                  >
                                    {fileName}
                                  </span>
                                  <button
                                    ...
                                    onClick={() => {
                                      setEditKtForm((prev) => ({
                                        ...prev,
                                        filesToDelete:
                                          prev.filesToDelete.includes(rawPath)
                                            ? prev.filesToDelete.filter(
                                                (p) => p !== rawPath,
                                              )
                  ...
                  <div className="exf-kt-form-row">
                    <div className="exf-kt-form-group exf-kt-form-group-half">
                      ...
                      <select
                        value={
                          editingKtId ? editKtForm.status : newKtForm.status
                        }
                        onChange={(e) =>
                          editingKtId
                            ? setEditKtForm({
                    ...
                    <div className="exf-kt-form-group exf-kt-form-group-half">
                      ...
                      <input
                        ...
                        value={
                          editingKtId
                            ? editKtForm.completedDate
                            : newKtForm.completedDate
                        }
                        onChange={(e) =>
                          editingKtId
                            ? setEditKtForm({
                ...
                <div className="exf-kt-modal-footer">
                  ...
                  <button
                    ...
                    disabled={
                      editingKtId
                        ? !editKtForm.topic.trim() ||
                          !editKtForm.description.trim()
                        : !newKtForm.topic.trim() ||
                          !newKtForm.description.trim()
                    }
