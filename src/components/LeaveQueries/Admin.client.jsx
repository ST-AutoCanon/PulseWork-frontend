...
const getLeaveTypeVariants = (value) => {
  const raw = String(value ?? "").trim();
  if (!raw) return [];

  const base = normalizeLeaveTypeKey(raw);
  const compact = base
    .replace(/\bleave\b/g, "")
    .replace(/\s+/g, " ")
    .trim();
  const variants = new Set([raw, raw.toLowerCase(), base, compact]);

  for (const candidate of [base, compact]) {
...
export default function Admin({ openPolicyId = null }) {
  ...
  const fetchEmployeeProfile = async (employeeId) => {
    ...
    for (const url of candidates) {
      try {
        ...
        const profileObject = Array.isArray(profile)
          ? profile[0]
          : typeof profile === "object"
            ? profile
            : null;

        if (!profileObject) continue;

        const gender =
          profileObject.gender ||
          profileObject.sex ||
          profileObject.Gender ||
          profileObject.personal_gender ||
          profileObject.employee_gender ||
          null;

        const dob =
          profileObject.dob ||
          profileObject.date_of_birth ||
          profileObject.dateOfBirth ||
          profileObject.employee_dob ||
          profileObject.personal_dob ||
          null;
  ...
  const filterGenderSpecificBalances = (balanceArray = [], profile = null) => {
    if (!Array.isArray(balanceArray)) return [];
    const gender = (profile?.gender || "").toString().toLowerCase();
    const dob = profile?.dob || null;
    const age = computeAge(dob);
  ...
  const augmentBalancesWithMenstrual = (balanceArray = [], profile = null) => {
    if (!Array.isArray(balanceArray)) return [];
    const copy = filterGenderSpecificBalances(balanceArray.slice(), profile);
    const idx = copy.findIndex((b) =>
      canonicalTypeMatch(b.type || b.label || "", "menstrual", "menstr"),
    );
    const gender = (profile?.gender || "").toString().toLowerCase();
  ...
  const loadLeaveBalance = async (employeeId, asOfDate = null) => {
    ...
    try {
      ...
      try {
        const profile = await fetchEmployeeProfile(employeeId);
        if (profile) {
          data = filterGenderSpecificBalances(
            augmentBalancesWithMenstrual(data, profile),
            profile,
          );
        }
      } catch (err) {}
  ...
  const handleUpdate = async (leaveId, query) => {
    ...
    if (upd.status === "Approved") {
      ...
      const qTypeVariants = getLeaveTypeVariants(
        query.leave_type ||
          query.leaveType ||
          query.leave_type_label ||
          query.leaveTypeLabel ||
      ...
      const balanceByNormalizedType = (balances || []).reduce((acc, b) => {
        const key = normalizeLeaveTypeKey(b.type || b.label || "");
        if (key) acc[key] = b;
        return acc;
      }, {});
      ...
      for (const variant of qTypeVariants) {
        const direct = balanceByNormalizedType[normalizeLeaveTypeKey(variant)];
        if (direct) {
          bal = direct;
          break;
        }
      ...
      if (!bal) {
        bal =
          (balances || []).find((r) => {
            const rowVariants = getLeaveTypeVariants(r.type || r.label || "");
            return rowVariants.some((rowVariant) =>
              qTypeVariants.some(
                (qVariant) =>
                  normalizeLeaveTypeKey(rowVariant) ===
                  normalizeLeaveTypeKey(qVariant),
              ),
            );
          }) || null;
      }
      ...
      if (!bal) {
        const typeMap = (leaveTypes || []).reduce((acc, lt) => {
          const variants = getLeaveTypeVariants(
            lt.key || lt.type || lt.label || "",
          );
          for (const variant of variants)
            acc[normalizeLeaveTypeKey(variant)] = lt;
          return acc;
        }, {});

        for (const variant of qTypeVariants) {
          const mapped = typeMap[normalizeLeaveTypeKey(variant)];
          if (mapped) {
            const mappedVariants = getLeaveTypeVariants(
              mapped.key || mapped.type || mapped.label || "",
            );
            const mappedBalance = (balances || []).find((r) => {
              const rowVariants = getLeaveTypeVariants(r.type || r.label || "");
              return rowVariants.some((rowVariant) =>
                mappedVariants.some(
                  (mVariant) =>
                    normalizeLeaveTypeKey(rowVariant) ===
                    normalizeLeaveTypeKey(mVariant),
                ),
              );
            });
            if (mappedBalance) {
