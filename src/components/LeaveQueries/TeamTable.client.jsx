...
export default function TeamTable({
  ...
  const getRemainingForLeave = async (leave) => {
    try {
      if (typeof loadLeaveBalance === "function") {
        const balances = await loadLeaveBalance(leave.employee_id, true);
        if (Array.isArray(balances)) {
          const targetVariants = getLeaveTypeVariants(leave.leave_type);
          const matches = balances.filter((r) => {
            const candidate = normalizeLeaveTypeKey(r?.type || r?.label || "");
            if (!candidate) return false;
            const candidateVariants = getLeaveTypeVariants(
              r?.type || r?.label || "",
            );
            return candidateVariants.some((v) => targetVariants.includes(v));
          });
          const best = matches.sort((a, b) => {
            const aNum = Number(a?.remaining ?? a?.allowance ?? 0);
            const bNum = Number(b?.remaining ?? b?.allowance ?? 0);
            return bNum - aNum;
          })[0];
          if (best && best.remaining !== undefined)
            return Number(best.remaining) || 0;
        }
      }
    } catch (e) {}
    if (leave.remaining !== undefined) return Number(leave.remaining) || 0;
