...
export default function useLeaveRequest() {
  ...
  const handleUpdate = async (leaveId, payload = null) => {
    ...
    try {
      ...
      const affectedEmployeeId =
        (leaveRequests.self || []).find(
          (row) => String(row.leave_id || row.id) === String(leaveId),
        )?.employee_id ||
        (leaveRequests.team || []).find(
          (row) => String(row.leave_id || row.id) === String(leaveId),
        )?.employee_id ||
      ...
      if (affectedEmployeeId) {
        setLeaveBalancesCache((prev) => {
          const next = { ...prev };
          delete next[affectedEmployeeId];
          return next;
        });
      }
  ...
  const fetchEmployeeProfile = async (targetEmployeeId) => {
    if (!targetEmployeeId) return null;

    const candidates = [
      `${BACKEND}/api/employee/${targetEmployeeId}`,
      `${BACKEND}/api/employees/${targetEmployeeId}`,
      `${BACKEND}/api/profile/${targetEmployeeId}`,
      `${BACKEND}/api/employee/profile/${targetEmployeeId}`,
      `${BACKEND}/api/employee-profile/${targetEmployeeId}`,
      `${BACKEND}/full/${targetEmployeeId}`,
    ];
    ...
    for (const url of candidates) {
      try {
        ...
        return {
          ...profile,
          gender: profile.gender || profile.sex || profile.Gender || null,
          dob:
            profile.dob ||
            profile.date_of_birth ||
            profile.dateOfBirth ||
            profile.employee_dob ||
            null,
        };
      } catch (err) {
  ...
  const isLeaveTypeVisibleForUser = (typeName = "") => {
    const gender = (userProfile?.gender || user?.gender || user?.sex || "")
      .toString()
      .toLowerCase();
    const dob = userProfile?.dob || user?.date_of_birth || user?.dob;
  ...
  const fetchLeaveTypes = async () => {
    try {
        ...
        const derived = (settings || [])
          .map((s) => {
            ...
            const label =
              ...
              (typeof s === "string"
                ? rawKey
                : s.display_name || rawKey || key);
            return {
              key: key ? key.toLowerCase().replace(/\s+/g, "_") : key,
              label,
            };
          })
          .filter((item) => isLeaveTypeVisibleForUser(item.key || item.label));
        const normalized = normalizeLeaveTypes(derived);
        setLeaveTypes(normalized);
        return normalized;
      }
      ...
      const json = await res.json();
      const arr = json?.data ?? (Array.isArray(json) ? json : []);
      const normalized = normalizeLeaveTypes(arr).filter((item) =>
        isLeaveTypeVisibleForUser(item.key || item.label),
      );
      setLeaveTypes(normalized);
      console.debug("fetchLeaveTypes -> normalized:", normalized);
      return normalized;
  ...
  const getProfileGenderContext = (profileOverride = null) => {
    const source = profileOverride || userProfile || user || {};
    const gender = (source?.gender || source?.sex || "")
      .toString()
      .toLowerCase();
    const dob = source?.dob || source?.date_of_birth || null;
    const age = computeAge(dob);
    return { gender, age };
  };

  const fetchLeaveBalance = async (profileOverride = null) => {
    if (!employeeId) return;
    try {
      const res = await fetch(
        `${BACKEND}/api/leave-policies/employee/${employeeId}/leave-balance`,
      ...
      arr = filterGenderSpecificBalances(
        augmentBalancesWithMenstrual(arr, profileOverride),
        profileOverride,
      );
  ...
  const filterGenderSpecificBalances = (
    balanceArray = [],
    profileOverride = null,
  ) => {
    if (!Array.isArray(balanceArray)) return [];
    const { gender, age } = getProfileGenderContext(profileOverride);

    return balanceArray.filter((b) => {
      const typeName = String(b?.type || b?.label || "").toLowerCase();
      if (typeName.includes("maternity")) return gender === "female";
      if (typeName.includes("menstrual")) {
        if (gender !== "female") return false;
        return age === null || (age >= 18 && age <= 52);
      }
      if (typeName.includes("paternity")) return gender === "male";
      return true;
    });
  };

  const augmentBalancesWithMenstrual = (
    balanceArray = [],
    profileOverride = null,
  ) => {
    if (!Array.isArray(balanceArray)) return [];
    const { gender, age } = getProfileGenderContext(profileOverride);
    const copy = filterGenderSpecificBalances(
      balanceArray.slice(),
      profileOverride,
    );
    const idx = copy.findIndex((b) =>
    ...
    const original = { ...copy[idx] };
    const annualCap = menstrualAnnualCap(original);
    ...
    if (!usedThisMonth && !alreadyEphemeralForThisMonth) {
      const baseAllowance = Math.min(
        Number(original.allowance ?? original.earned ?? 0),
        annualCap,
      );
      const baseUsed = Number(original.used ?? 0);
      const baseRemaining = Number(
        original.remaining ?? Math.max(baseAllowance - baseUsed, 0),
      );
      const nextAllowance = Math.min(baseAllowance + 1, annualCap);
      const nextRemaining = Math.min(
        Math.max(baseRemaining + 1, 0),
        Math.max(annualCap - baseUsed, 0),
      );
      original.allowance = nextAllowance;
      original.remaining = nextRemaining;
      original.ephemeral = true;
      original.ephemeral_month = currentMonth;
      original.ephemeral_year = currentYear;
      original.carry_forward = 0;
  ...
  const loadLeaveBalance = async (employeeIdToLoad, forceRefresh = false) => {
    if (!employeeIdToLoad) return [];
    if (!forceRefresh && leaveBalancesCache[employeeIdToLoad])
      return leaveBalancesCache[employeeIdToLoad];
    ...
    try {
      ...
      const targetProfile =
        employeeIdToLoad === employeeId
          ? profile || userProfile
          : (await fetchEmployeeProfile(employeeIdToLoad)) || userProfile;
      arr = filterGenderSpecificBalances(
        augmentBalancesWithMenstrual(arr, targetProfile),
        targetProfile,
      );
      setLeaveBalancesCache((prev) => ({ ...prev, [employeeIdToLoad]: arr }));
      return arr;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        await fetchLeaveBalance(profile || userProfile);
      } catch {}
