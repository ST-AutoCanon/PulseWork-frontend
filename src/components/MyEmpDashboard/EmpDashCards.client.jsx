...
const getLatestPunchPayload = (result) => {
...
export default function EmpDashCards() {
  ...
  const handlePunch = async () => {
    ...
    try {
      ...
      const payload = getLatestPunchPayload(result);

      const lateLoginDetected =
        Boolean(
          payload?.lateLogin ??
          payload?.late_login ??
          payload?.data?.lateLogin ??
          payload?.data?.late_login,
        ) || false;
      ...
      const messageText =
        ...
        result?.message ||
        (lateLoginDetected
          ? "Late login recorded successfully."
          : "Punch completed successfully.");

      if (lateLoginDetected) {
        setLateLoginPopup(messageText);
      }
