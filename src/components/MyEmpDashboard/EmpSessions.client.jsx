...
const EmpSessions = () => {
  ...
  // ✅ MUST live inside component
  function applySessionResult(result) {
    setTotalWorkSeconds(result.workSec);
    setTotalIdleSeconds(result.idleSec);
    setRemainingSeconds(result.remaining);
    setChartData(buildChartFromSegments(result.merged));
  }
  ...
  useEffect(() => {
    ...
    const fetchSessionData = async () => {
      ...
      try {
        const data = await fetchTodayPunch(employeeId, BACKEND, headers);
        if (cancelled) return;
        ...
        const result = processPunchData(data.data || []);
        applySessionResult(result);
      } catch (err) {
        handleSessionError(err, setError);
      } finally {
        setLoading(false);
      }
    };
...
async function fetchTodayPunch(employeeId, BACKEND, headers) {
...
function processPunchData(punchData) {
  ...
  handleInitialIdle(punchData, segments, (v) => (idleSec += v));
  ...
  return { workSec, idleSec, remaining, merged };
}

function handleInitialIdle(punchData, segments, addIdle) {
  if (punchData.length === 0) return;
...
function handleSessionError(err, setError) {
  setError(
