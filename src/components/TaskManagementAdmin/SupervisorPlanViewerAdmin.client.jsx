  ...
  useEffect(() => {
    ...
    const fetchHolidays = async () => {
      ...
      try {
        ...
        setHolidays(
          Array.isArray(response.data.holidays) ? response.data.holidays.map((h) => h.date) : []
        );
      } catch (err) {
        setHolidays([]);
      } finally {
  ...
  useEffect(() => {
    ...
    const fetchTasks = async () => {
      ...
      try {
        ...
        setTasks(taskData);
        if (taskData.length > 0) {
          const weekIds = [...new Set(taskData.map((t) => t.week_id))].sort((a, b) => a - b);
          setSelectedWeekId(weekIds[weekIds.length - 1] || null);
        } else {
          setSelectedWeekId(null);
        }
...
const updateConfig = async () => {
  ...
  try {
    ...
    showAlert("Failed to update configuration: " + (err.response?.data?.message || err.message));
  } finally {
    setLoadingConfig(false);
  }
};
  ...
  const getTaskDateStyle = (dateString, employeeId) => {
    if (!dateString) return { className: "supervisor-plan-task-date supervisor-plan-task-date-regular", tooltip: "N/A" };
    const taskDate = new Date(dateString);
    taskDate.setHours(0, 0, 0, 0);
  ...
  // ────────────────────── 7. useMemo ──────────────────────
  const weekIds = useMemo(() => [...new Set(tasks.map((t) => t.week_id))].sort((a, b) => a - b), [tasks]);
  ...
  // ────────────────────── 8. EARLY RETURNS ──────────────────────
  if (!hydrated) return <div className="supervisor-plan-admin-wrapper">Loading user...</div>;
  if (!user) return <div className="supervisor-plan-admin-wrapper"><div className="supervisor-plan-admin-error-message">Please <a href="/login">log in</a>.</div></div>;
  if (!supervisorId) return <div className="supervisor-plan-admin-wrapper"><div className="supervisor-plan-admin-error-message">{error || "Supervisor ID missing."} <a href="/login">Log in again</a>.</div></div>;

  // ────────────────────── 9. RENDER ──────────────────────
  return (
    <div className="supervisor-plan-admin-wrapper">
      <Modal isVisible={alertModal.isVisible} onClose={() => setAlertModal({ isVisible: false, message: "" })} buttons={[{ label: "OK", onClick: () => setAlertModal({ isVisible: false, message: "" }) }]}>
        <p>{alertModal.message}</p>
      </Modal>

      {configModal.isVisible && (
        <div className="supervisor-plan-admin-modal-overlay" onClick={() => setConfigModal((prev) => ({ ...prev, isVisible: false }))}>
          <form className="supervisor-plan-admin-modal" onClick={(e) => e.stopPropagation()}>
            <h3 className="supervisor-plan-admin-modal-title">Update Freeze Days</h3>
            <div className="supervisor-plan-admin-config-modal-content">
              <div className="supervisor-plan-admin-config-input-group">
            ...
            <div className="supervisor-plan-admin-modal-buttons">
              <button type="button" className="supervisor-plan-admin-modal-button supervisor-plan-admin-modal-button-cancel" onClick={() => setConfigModal((prev) => ({ ...prev, isVisible: false }))} disabled={loadingConfig}>
                Cancel
              </button>
              <button type="button" className="supervisor-plan-admin-modal-button supervisor-plan-admin-modal-button-save" onClick={updateConfig} disabled={loadingConfig}>
                Save
      ...
      <div className="supervisor-plan-admin-header">
        <button className="supervisor-plan-admin-config-button" onClick={fetchConfig} disabled={loadingConfig} style={{ position: "absolute", top: "10px", right: "10px" }}>
          {loadingConfig ? "Loading..." : "Update Freeze Days"}
        </button>
      </div>
      ...
      <div className="supervisor-plan-admin-employee-list">
        <h3>Employees</h3>
        <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search employees by name" className="supervisor-plan-admin-search-bar" />
        {error && <p style={{ color: "red" }}>{error}</p>}
        {loadingEmployees || loadingHolidays || loadingLeaves ? (
          <p>Loading employees...</p>
        ) : filteredEmployees.length === 0 ? (
          ...
          <ul className="supervisor-plan-admin-employee-scroll">
            {filteredEmployees.map((emp) => (
              <li key={emp.employee_id} className={selectedEmployee === emp.employee_id ? "supervisor-plan-admin-active" : ""} onClick={() => setSelectedEmployee(emp.employee_id)}>
                {emp.employee_name}
              </li>
            ))}
          </ul>
      ...
      <div className="supervisor-plan-admin-task-details">
        {loadingTasks || loadingProjects ? (
          ...
          <>
            <div className="supervisor-plan-admin-week-navigation">
              <button className="supervisor-plan-admin-nav-button" onClick={goToPreviousWeek} disabled={currentWeekIndex <= 0}> &lt;</button>
              <span className="supervisor-plan-admin-week-label">{formatWeekId(selectedWeekId)}</span>
              <button className="supervisor-plan-admin-nav-button" onClick={goToNextWeek} disabled={currentWeekIndex >= weekIds.length - 1}> &gt;</button>
            </div>
            <div className="supervisor-plan-admin-tasks-container">
              {weekDays.map(({ dateStr, dateDisplay }) => {
                const dayTasks = tasksByDate[dateStr] || [];
                ...
                return (
                  <div key={dateStr} className="supervisor-plan-admin-day-group">
                    <div className="supervisor-plan-admin-day-header">
                      <span className={dateStyle.className} title={dateStyle.tooltip}>{dateDisplay}</span>
                    </div>
                    {dayTasks.length === 0 ? (
                      <p className="supervisor-plan-admin-no-tasks">No tasks assigned for this day.</p>
                    ) : (
                          ...
                          <div key={task.task_id} className={`supervisor-plan-admin-task-card ${isFrozen ? "supervisor-plan-admin-task-frozen" : ""}`}>
                            <div className="supervisor-plan-admin-task-header">
                              <div className="supervisor-plan-admin-task-title">
                                {effectiveReviewStatus === "struck" ? (
                                  <>
                                    <span style={{ textDecoration: "line-through", color: "#a0a0a0" }}>{task.task_name}</span>
                                    {task.replacement_task && <span style={{ color: "#007bff", marginLeft: "8px" }}>Arrow right {task.replacement_task}</span>}
                                  </>
                                ) : (
                                  task.task_name
                                )}
                              ...
                              <div className="supervisor-plan-admin-task-meta">
                                ...
                                <span className={taskDateStyle.className} title={taskDateStyle.tooltip}>{formatDate(task.task_date)}</span>
                                <div className="supervisor-plan-admin-project-circle-wrapper">
                                  <span className="supervisor-plan-admin-project-circle">{task.project_id || "N/A"}</span>
                                  <div className="supervisor-plan-admin-tooltip">{task.project_name || "Unknown"}</div>
                                </div>
                                <div className="supervisor-plan-admin-status-dot-wrapper">
                                  <span className="supervisor-plan-admin-status-dot" style={{ backgroundColor: statusColor(task.emp_status) }}></span>
                                  <div className="supervisor-plan-admin-tooltip">{statusLabel(task.emp_status)}</div>
                                </div>
                              </div>
                            </div>
                            <div className="supervisor-plan-admin-task-body">
                            ...
                            {isFrozen && <div className="supervisor-plan-admin-frozen-message">This task is suspended and frozen. No edits allowed.</div>}
                            <div className={`supervisor-plan-admin-edit-section ${isFrozen ? "supervisor-plan-admin-edit-section-disabled" : ""}`}>
                              <label>Project: <select value={task.project_id || ""} onChange={(e) => updateTaskField(task.task_id, "project", e.target.value)} disabled={isFrozen}>
                                <option value="">Select Project</option>
                                {Object.entries(projects).map(([id, name]) => (
                                  <option key={id} value={id}>{id} - {name}</option>
                                ))}
                              </select></label>
                              <label>Update: <select value={task.sup_status || "incomplete"} onChange={(e) => updateTaskField(task.task_id, "sup_status", e.target.value)} disabled={isFrozen}>
                                <option value="completed">Completed</option>
                                <option value="add on">Add On</option>
                                <option value="re-work">Re-work</option>
                                <option value="incomplete">Incomplete</option>
                              </select></label>
                              <label>Feedback: <input type="text" value={task.sup_comment || ""} onChange={(e) => updateTaskField(task.task_id, "sup_comment", e.target.value)} placeholder="Add comment" disabled={isFrozen} /></label>
                              {showReviewSelect && (
                                <label>Review: <select value={task.sup_review_status || "pending"} style={{ color: getReviewStatusColor(task.sup_review_status) }} onChange={(e) => handleReviewChange(task.task_id, e.target.value)} disabled={isFrozen}>
                                  <option value="pending">Pending</option>
                                  <option value="approved">Approved</option>
                                  <option value="struck">Update task</option>
                                  <option value="suspended_review">Suspended</option>
                              ...
                              {effectiveReviewStatus === "struck" && (
                                <label>Updated task: <input type="text" value={task.replacement_task || ""} onChange={(e) => updateTaskField(task.task_id, "replacement_task", e.target.value)} placeholder="Enter updated task" disabled={isFrozen} /></label>
                              )}
                              {effectiveReviewStatus !== "pending" && (
                                <label>Rating:
                                  <div className="supervisor-plan-admin-star-rating">
                                    {[1, 2, 3, 4, 5].map((star) => (
                                      <span key={star} className={`supervisor-plan-admin-star ${task.star_rating >= star ? "filled" : ""}`} onClick={() => !isFrozen && updateTaskField(task.task_id, "star_rating", star)} style={{ cursor: isFrozen ? "not-allowed" : "pointer" }}>★</span>
                                    ))}
                                  </div>
                                </label>
                              )}
                              <button className="supervisor-plan-admin-update-task-button" onClick={() => saveTaskField(task.task_id)} disabled={isFrozen}>Update</button>
                            </div>
                          </div>
                        );
                      })
