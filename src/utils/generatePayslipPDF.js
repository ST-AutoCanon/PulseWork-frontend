import { jsPDF } from "jspdf";
import autoTable from "jspdf-autotable";

const convertNumberToWords = (num) => {
  if (!num) return "Zero";
  const ones = [
    "",
    "One",
    "Two",
    "Three",
    "Four",
    "Five",
    "Six",
    "Seven",
    "Eight",
    "Nine",
  ];
  const teens = [
    "Eleven",
    "Twelve",
    "Thirteen",
    "Fourteen",
    "Fifteen",
    "Sixteen",
    "Seventeen",
    "Eighteen",
    "Nineteen",
  ];
  const tens = [
    "",
    "Ten",
    "Twenty",
    "Thirty",
    "Forty",
    "Fifty",
    "Sixty",
    "Seventy",
    "Eighty",
    "Ninety",
  ];
  const thousands = ["", "Thousand", "Lakh", "Crore"];

  let words = "";
  const getWords = (n, index) => {
    if (n > 0) {
      if (n < 10) words += ones[n] + " ";
      else if (n < 20) words += teens[n - 11] + " ";
      else words += tens[Math.floor(n / 10)] + " " + ones[n % 10] + " ";
      words += thousands[index] + " ";
    }
  };

  getWords(Math.floor(num / 10000000), 3); // Crore
  getWords(Math.floor((num % 10000000) / 100000), 2); // Lakh
  getWords(Math.floor((num % 100000) / 1000), 1); // Thousand
  getWords(Math.floor((num % 1000) / 100), 0); // Hundreds

  if (num % 100 > 0) getWords(num % 100, 0);
  return words.trim() + " only";
};

function generatePayslipPDF(
  payrollData = {},
  selectedDate = {},
  bankDetails = {},
  attendance = {},
  employeeDetails = {},
  download = false
) {
  // Defensive logging (helpful while debugging)
  // console.debug({ payrollData, selectedDate, bankDetails, attendance, employeeDetails });

  // Basic validation (still allow missing fields, but warn)
  if (!payrollData || !selectedDate) {
    console.error("Missing payrollData or selectedDate", {
      payrollData,
      selectedDate,
    });
    return null;
  }

  // --- NORMALIZE / FALLBACKS ---
  // Payroll values - accept multiple possible keys that callers may use
  const employeeName = (
    payrollData.employee_name ||
    payrollData.employeeName ||
    payrollData.name ||
    ""
  ).toString();
  const employeeId = (
    payrollData.employee_id ||
    payrollData.employeeId ||
    payrollData.emp_id ||
    ""
  ).toString();
  const designation = (
    payrollData.designation ||
    payrollData.designation ||
    ""
  ).toString();

  const basicSalary = Number(
    payrollData.basic_salary ??
      payrollData.basic ??
      payrollData.basicSalary ??
      payrollData.basic_amount ??
      0
  );

  const hra = Number(
    payrollData.hra ?? payrollData.HRA ?? payrollData.hra_amount ?? 0
  );

  const allowance = Number(
    payrollData.allowance ??
      payrollData.other_allowance ??
      payrollData.otherAllowance ??
      0
  );

  const pf = Number(payrollData.pf ?? payrollData.pf_amount ?? 0);

  const insurance = Number(
    payrollData.insurance ??
      payrollData.esi_insurance ??
      payrollData.esiInsurance ??
      0
  );

  const pt = Number(
    payrollData.pt ??
      payrollData.professional_tax ??
      payrollData.professionalTax ??
      0
  );

  const tds = Number(payrollData.tds ?? payrollData.TDS ?? 0);

  const totalEarnings = Number(
    payrollData.total_earnings ??
      payrollData.gross_earnings ??
      payrollData.grossEarnings ??
      basicSalary + hra + allowance
  );

  const totalDeductions = Number(
    payrollData.total_deductions ??
      payrollData.totalDeductions ??
      payrollData.totalDeduction ??
      pf + insurance + pt + tds
  );

  const netSalary = Number(
    payrollData.net_salary ??
      payrollData.netSalary ??
      payrollData.net_salary_amount ??
      payrollData.net_salary_amount ??
      totalEarnings - totalDeductions
  );

  // Bank details fallbacks
  const accountNumber =
    bankDetails.account_number ??
    bankDetails.account_no ??
    bankDetails.accountNo ??
    bankDetails.account ??
    "";
  const bankName = bankDetails.bank_name ?? bankDetails.bankName ?? "";
  const uin =
    bankDetails.uin_number ??
    bankDetails.uin_no ??
    bankDetails.uin ??
    payrollData.uin_number ??
    payrollData.uin_no ??
    payrollData.uin ??
    employeeDetails.uin_number ??
    employeeDetails.uin_no ??
    employeeDetails?.uan_number ??
    employeeDetails?.uan_no ??
    employeeDetails?.uan ??
    employeeDetails?.uanNumber ??
    "";
  const esiNumber =
    bankDetails.esi_number ??
    bankDetails.esi ??
    payrollData.esi_number ??
    payrollData.esi ??
    employeeDetails.esi_number ??
    "";
  const pfNumber =
    bankDetails.pf_number ??
    bankDetails.pfNumber ??
    payrollData.pf_number ??
    payrollData.pf ??
    employeeDetails.pf_number ??
    "";

  // Attendance fallbacks
  const totalWorkingDays =
    attendance.total_working_days ??
    attendance.working_days ??
    attendance.workingDays ??
    attendance.totalWorkingDays ??
    "";
  const leaveCount =
    attendance.leave_count ??
    attendance.leaves_taken ??
    attendance.leavesTaken ??
    attendance.leaveCount ??
    0;

  // Employee details fallbacks (PAN etc.)
  const gender = employeeDetails.gender ?? payrollData.gender ?? "N/A";
  const panNumber =
    bankDetails.pan_number ??
    bankDetails.panNumber ??
    employeeDetails.pan_number ??
    employeeDetails.panNumber ??
    payrollData.pan_number ??
    payrollData.panNumber ??
    "";
  const joiningDate =
    payrollData.joining_date ??
    payrollData.date_of_joining ??
    employeeDetails.date_of_joining ??
    payrollData.dateOfJoining ??
    "";

  // Convert netSalary to words
  const formattedNetSalary = Math.round(netSalary || 0).toString();
  const netSalaryWords = convertNumberToWords(
    parseInt(formattedNetSalary, 10) || 0
  );

  // --- BUILD PDF ---
  const doc = new jsPDF();
  const pageWidth = doc.internal.pageSize.getWidth();
  const margin = 10;
  const maxWidth = pageWidth - margin * 2;

  doc.setFontSize(12);

  // header background
  doc.setFillColor(15, 102, 121);
  doc.rect(margin, 10, maxWidth, 25, "F");

  // optional logo: keep try/catch (your base64 may be large)
  try {
    const logoBase64 =
      "data:image/png;base64,";
    doc.addImage(logoBase64, "PNG", 11, 11, 23, 23);
  } catch (e) {
    // ignore if image fails
  }

  // header text
  doc.setTextColor(255, 255, 255);
  doc.setFontSize(16).setFont("helvetica", "bold");
  doc.text("Sukalpa Tech Solutions Pvt Ltd", pageWidth - margin - 5, 20, {
    align: "right",
  });
  doc.setFontSize(10).setFont("helvetica", "normal");
  doc.text(
    "#71, Sarathy Nagar, Near Sahyadri Nagar, Belagavi - 591108",
    pageWidth - margin - 5,
    27,
    { align: "right" }
  );
  doc.text(
    "Phone no.: +91 831 406 9203  Email: hr@sukalpatechsolutions.com",
    pageWidth - margin - 5,
    34,
    { align: "right" }
  );
  doc.setTextColor(0, 0, 0);

  // title
  doc.setFontSize(14).setFont("helvetica", "bold");
  const monthNames = [
    "January",
    "February",
    "March",
    "April",
    "May",
    "June",
    "July",
    "August",
    "September",
    "October",
    "November",
    "December",
  ];
  const formattedMonthYear = `${
    monthNames[(selectedDate.month ?? selectedDate.monthNo ?? 1) - 1 || 0]
  } ${selectedDate.year ?? new Date().getFullYear()}`;
  doc.text(`Payslip for ${formattedMonthYear}`, pageWidth / 2, 60, {
    align: "center",
  });

  // Employee details (left)
  doc.setFontSize(10).setFont("helvetica", "bold");
  doc.text("Employee Name:", 20, 75);
  doc
    .setFont("helvetica", "normal")
    .text(
      employeeName ? employeeName.toUpperCase().substring(0, 30) : "N/A",
      60,
      75
    );
  doc.setFont("helvetica", "bold").text("Gender:", 20, 82);
  doc
    .setFont("helvetica", "normal")
    .text((gender || "N/A").toString().toUpperCase(), 60, 82);
  doc.setFont("helvetica", "bold").text("Date of Joining:", 20, 89);
  doc.setFont("helvetica", "normal").text(joiningDate || "N/A", 60, 89);
  doc.setFont("helvetica", "bold").text("No. of Working Days:", 20, 96);
  doc
    .setFont("helvetica", "normal")
    .text(totalWorkingDays !== "" ? String(totalWorkingDays) : "N/A", 60, 96);
  doc.setFont("helvetica", "bold").text("UAN No:", 20, 103);
  doc.setFont("helvetica", "normal").text(uin || "N/A", 60, 103);

  // Right side PAN:

  doc.setFont("helvetica", "bold").text("ESI Number:", 20, 110);
  doc.setFont("helvetica", "normal").text(esiNumber || "N/A", 60, 110);

  // Right side
  doc.setFont("helvetica", "bold").text("Employee ID:", 125, 75);
  doc.setFont("helvetica", "normal").text(employeeId || "N/A", 155, 75);
  doc.setFont("helvetica", "bold").text("Designation:", 125, 82);
  doc
    .setFont("helvetica", "normal")
    .text(
      designation ? designation.toUpperCase().substring(0, 30) : "N/A",
      155,
      82
    );
  doc.setFont("helvetica", "bold").text("Account No:", 125, 89);
  doc
    .setFont("helvetica", "normal")
    .text(
      accountNumber ? `${accountNumber} (${bankName || "N/A"})` : "N/A",
      155,
      89
    );
  doc.setFont("helvetica", "bold").text("Leaves Taken:", 125, 96);
  doc.setFont("helvetica", "normal").text(String(leaveCount || 0), 155, 96);
  doc.setFont("helvetica", "bold").text("PAN Number:", 125, 103);
  doc.setFont("helvetica", "normal").text(panNumber || "N/A", 155, 103);
  doc.setFont("helvetica", "bold").text("PF Number:", 125, 110);
  doc.setFont("helvetica", "normal").text(pfNumber || "N/A", 155, 110);

  // Earnings & Deductions table
  const earningsRows = [
    ["Basic", basicSalary.toFixed(2), "PF", pf.toFixed(2)],
    ["HRA", hra.toFixed(2), "ESI/Insurance", insurance.toFixed(2)],
    [
      "Other Allowance",
      allowance.toFixed(2),
      "Professional Tax",
      pt.toFixed(2),
    ],
  ];

  earningsRows.push(["", "", "TDS", tds.toFixed(2)]);
  earningsRows.push([
    "Gross Earnings",
    totalEarnings.toFixed(2),
    "Total Deductions",
    totalDeductions.toFixed(2),
  ]);

  autoTable(doc, {
    startY: 115,
    head: [["Earnings", "Amount", "Deductions", "Amount"]],
    body: earningsRows,
    theme: "grid",
    styles: {
      fontSize: 9,
      lineColor: [0, 0, 0],
      lineWidth: 0.3,
      overflow: "linebreak",
    },
    headStyles: {
      fillColor: [15, 102, 121],
      textColor: [255, 255, 255],
      fontSize: 9,
    },
    columnStyles: {
      0: { cellWidth: 50 },
      1: { cellWidth: 40, halign: "right" },
      2: { cellWidth: 50 },
      3: { cellWidth: 50, halign: "right" },
    },
    margin: { left: margin, right: margin },
  });

  // Net salary table
  const startYForNet = doc.lastAutoTable ? doc.lastAutoTable.finalY + 4 : 160;
  autoTable(doc, {
    startY: startYForNet,
    body: [
      ["Net Salary", netSalary.toFixed(0)],
      ["Net Salary (in words)", netSalaryWords.substring(0, 200)],
    ],
    theme: "grid",
    styles: {
      fontSize: 9,
      fontStyle: "bold",
      lineColor: [0, 0, 0],
      lineWidth: 0.3,
      overflow: "linebreak",
    },
    columnStyles: { 0: { cellWidth: 50 }, 1: { cellWidth: 140 } },
    margin: { left: margin },
  });

  const footerY = doc.lastAutoTable ? doc.lastAutoTable.finalY + 10 : 200;
  doc
    .setFontSize(9)
    .text(
      "This is a system-generated payslip, no signature required.",
      margin,
      footerY
    );

  // Always return blob so callers can preview / validate
  const blob = doc.output("blob");

  // DO NOT auto-download inside this helper. Return the blob so caller decides.
  return blob;
}

export default generatePayslipPDF;
